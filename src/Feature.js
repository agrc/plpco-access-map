...
const RelatedRecord = ({ feature, table }) => {
  ...
  React.useEffect(() => {
    const giddyUp = async () => {
      const { Feature, PopupTemplate } = await loadModules();
      feature.popupTemplate = PopupTemplate.fromJSON(table.popupInfo);

      new Feature({ container: container.current, graphic: feature });
    }
    if (feature) {
      giddyUp();
    }
...
const RelatedRecordContainer = ({ relatedRecordInfo }) => {
  ...
  const onClick = () => {
    ...
    $(container.current).collapse('toggle');
  };

  return (
    <div>
      <button className="btn btn-link" onClick={onClick} data-toggle="collapse" data-target={containerId}>
        { isOpen ?
          <svg width="1em" height="1em" viewBox="0 0 20 20" className="bi bi-caret-down" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
          ...
          <svg width="1em" height="1em" viewBox="0 0 20 20" className="bi bi-caret-right" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
      ...
      <div ref={container} id={containerId} className="collapse">
